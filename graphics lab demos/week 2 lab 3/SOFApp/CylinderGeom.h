...
class CylinderGeometry : public GeometryHelper
{
	...
	void CreateGeometry(float height, int nDivisions)
	{
		...
		for (int i = 0; i < nDivisions - 2; i++) {
			...
			uvs.push_back(circleUV[i0]);
			uvs.push_back(circleUV[i1]);
			uvs.push_back(circleUV[i2]);
		...
		//sides
		for (int i = 0; i < nDivisions; i++) {
			glm::vec3 pos[4]; glm::vec3 norms[4];  glm::vec3 tang[4];  glm::vec2 uv[4];
			//positions
			...
			norms[0] = circle[i];
			norms[1] = circle[i];
			norms[2] = circle[(i + 1) % nDivisions];
			norms[3] = circle[(i + 1) % nDivisions];
			//tangents
			tang[0] = glm::cross(norms[0], glm::vec3(0, 1, 0));
			tang[1] = glm::cross(norms[1], glm::vec3(0, 1, 0));
			tang[2] = glm::cross(norms[2], glm::vec3(0, 1, 0));
			tang[3] = glm::cross(norms[3], glm::vec3(0, 1, 0));
			//uv
			...
			for (int i = 0; i < 6; i++)
			{
				...
				normals.push_back(norms[triangles[i]]);
...
public:
	CylinderGeometry(float height=1.0f, int nDivisions =16) { CreateGeometry(height, nDivisions); }
