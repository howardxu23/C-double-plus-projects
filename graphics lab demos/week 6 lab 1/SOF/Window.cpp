...
float SOF::Window::mouseX;
float SOF::Window::mouseY;
...
glm::vec2 SOF::Window::GetMousePos()
{
	return glm::vec2(mouseX, mouseY);
}

glm::vec2 SOF::Window::GetMousePosNormalized()
{
	return glm::vec2(mouseX / width, mouseY / height);
}
...
void SOF::Window::Run()
{
	...
	while (!glfwWindowShouldClose(pWindow)) {
		...
		mouseX = fmin(width,fmax(0.0f,(float)x));
		mouseY = fmin(height,fmax(0.0f,(float)y));
