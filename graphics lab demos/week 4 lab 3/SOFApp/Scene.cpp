...
Node* Scene()
{
	...
	TransformNode* patchTrans = new TransformNode(glm::scale(glm::translate(glm::mat4(), glm::vec3(0, 5, 0)), glm::vec3(5, 5, 5)));
	//TexturedLit* patch = new TexturedLit(BezierPatch(bezPoints, 8), "textures/grid.jpg");
	TexturedLit* patch = new TexturedLit(Teapot(8), "textures/grid.jpg");
	GeometryNode* geomNode = new GeometryNode(patch);
	light->AddChild(patchTrans);
	patchTrans->AddChild(geomNode);
